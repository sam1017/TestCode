...
public class SlowMotionItem {
	...
	//Xiaoyh@Itel-Mobile fix the HangOut BUG by abroad at 2016/09/22
    /*private String getSlowMotionInfoFromDB(final Context context, final Uri uri) {
        ...
        try {
            ...
            cursor = context.getContentResolver().query(uri,new String[] { Video.Media.SLOW_MOTION_SPEED }, null, null, null);

            //transsion 02758 modified for bug:2430
			if (cursor == null || cursor.getColumnCount() == 0) {
                cursor = context.getContentResolver().query(MediaStore.Video.Media.EXTERNAL_CONTENT_URI,new String[] { Video.Media.SLOW_MOTION_SPEED }, where, null, null);
            }
            MtkLog.v(TAG, "getSlowMotionInfoFromDB() cursor="+ (cursor == null ? "null" : cursor.getCount()));
    ...
    private String getSlowMotionInfoFromDB(final Context context, final Uri uri) {
		...
		try {
			String str = Uri.decode(uri.toString());
			str = str.replaceAll("'", "''");
			MtkLog.v(TAG, "Uri str =" + str);
			final String where = "_data LIKE '%" + str.replaceFirst("file:///", "") + "'";
			String scheme = uri.getScheme();
			if (scheme == null) {
				return data;
			}
			if (scheme.equals("content") && uri.getAuthority().equals(MediaStore.AUTHORITY)) {
				cursor = context.getContentResolver().query(uri,new String[] { Video.Media.SLOW_MOTION_SPEED }, null, null, null);
			} else if (scheme.equals("file")) {
				cursor = context.getContentResolver().query(
				MediaStore.Video.Media.EXTERNAL_CONTENT_URI,new String[] { Video.Media.SLOW_MOTION_SPEED }, where, null, null);
			}
