...
public class MtkVideoTexture extends VideoSurfaceTexture implements ScreenModeListener,
    ...
    // The mRatio is used for zoom in/out in full screen or crop screen mode
    ...
    public boolean draw(MGLCanvas canvas, int x, int y, int width, int height) {
        ...
        synchronized (this) {
            ...
            if (!mIsInFilmMode) {
                // calculate ratio
                if ((float) mVideoWidth / mVideoHeight > (float) screenWidth / screenHeight) {
                    mRatio = (float) width / screenWidth;
                } else {
                    mRatio = (float) height / screenHeight;
                }
                ...
                case ScreenModeManager.SCREENMODE_FULLSCREEN:
                    width = Math.round(screenWidth * mRatio);
                    height = Math.round(screenHeight * mRatio);
                    break;
                case ScreenModeManager.SCREENMODE_CROPSCREEN:
                    ...
                    width = Math.round(width * mRatio);
                    height = Math.round(height * mRatio);
                    break;
                }
            }
            canvas.scale(1, -1, 1);
