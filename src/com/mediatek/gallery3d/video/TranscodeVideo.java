...
public class TranscodeVideo {
    ...
    // transcode tmp folder name.
    private static final String TEMP_FOLDER_NAMER = "/.sTempGallery/";
    ...
    // Prevent two or more stop task running at the same time
    ...
    private void startTranscode() {
        ...
        if (mItem.getSpeed() == SlowMotionItem.SLOW_MOTION_ONE_SIXTEENTH_SPEED) {
            Toast.makeText(mContext.getApplicationContext(),
                    mContext.getString(R.string.not_support_share_hint_for_16x), Toast.LENGTH_LONG)
                    .show();
            return;
        }
        showProgressDialog();
        mSlowMotionInfo = mItem.getSlowMotionInfo();
        // here should check whether the video has been transacted with same
        // parameter or not.
        if (isNeedTranscode(mUri)) {
            ...
            ((MovieActivity)mContext).setBeamUriForSlowMotion(mNewVideoUri);
