        ...
        if (speed == SlowMotionItem.SLOW_MOTION_ONE_SIXTEENTH_SPEED) {
            if (mIsSingleShare && mTotalShareCounts == 1) {
                MtkLog.v(TAG,
                        ...
                        " is 16x, show toast and return");
                promptMessage(R.string.not_avaliable_16x_single_share);
                return null;
            } else {
                MtkLog.v(TAG, "downgrade speed to 1/4x");
                speed = SlowMotionItem.SLOW_MOTION_QUARTER_SPEED;
                promptMessage(R.string.not_avaliable_16x_multi_share);
            }
        }
