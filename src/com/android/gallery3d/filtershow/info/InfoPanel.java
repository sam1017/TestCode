...
public class InfoPanel extends DialogFragment {
    ...
    private String createStringFromIfFound(ExifTag exifTag, int tag, int str) {
        ...
        if (tagId == ExifInterface.getTrueTagKey(tag)) {
            ...
            // transsion begin, IB-02533, xieweiwei, add, 2016.09.17
            // change yyyy:MM:dd kk:mm:ss to yyyy-MM-dd kk:mm:ss
            String datetime = changeDateTimeFormat(exifTag, ExifInterface.TAG_DATE_TIME_ORIGINAL);
            if (datetime != null) {
                exifString += datetime;
            } else {
            // transsion end
    ...
    // transsion begin, IB-02533, xieweiwei, add, 2016.09.17
    // change yyyy:MM:dd kk:mm:ss to yyyy-MM-dd kk:mm:ss
    public String changeDateTimeFormat(ExifTag exifTag, int tag) {
        if (exifTag != null && tag == ExifInterface.TAG_DATE_TIME_ORIGINAL) {
            String datetime = exifTag.forceGetValueAsString();
            char ch;
            boolean valid = true;
            String result = "";
            final char chs[] = new char[] {'d', 'd', 'd', 'd', ':', 'd', 'd', ':',
                'd', 'd', ' ', 'd', 'd', ':', 'd', 'd', ':', 'd', 'd', '\n'};
            if (datetime.length() != chs.length) {
                valid = false;
            } else {
                boolean findBlank = false;
                for (int i = 0; i < datetime.length(); i++) {
                    ch = datetime.charAt(i);
                    if ((chs[i] == 'd' && ch >= '0' && ch <= '9')
                        || (chs[i] == ':' && ch == ':')
                        || (chs[i] == ' ' && ch == ' ')
                        || (chs[i] == '\n')) {
                        if (!findBlank) {
