...
/// @}
    ...
    private void updateExifData(ExifInterface exif, long time) {
        ...
        if (FeatureConfig.sSupportRefocus && sClearRefocusFlag) {
            exif.deleteTag(ExifInterface.TAG_REFOCUS);
            Log.i(LOGTAG,
                "<updateExifData> <refocus> delete refocus exif tag when saving edited image!");
        }
        /// @}
    }
    ...
    public Uri processAndSaveImage(ImagePreset preset, boolean flatten,
        ...
        */
        ...
        if (null != origPicSize) {
            ...
            Log.d(LOGTAG, "<processAndSaveImage> sIsLowRamDevice "
                    + FeatureConfig.sIsLowRamDevice + ", decode sample size "
        ...
        while (noBitmap) {
            try {
                ...
                /*Bitmap bitmap =ImageLoader.loadOrientedBitmapWithBackouts(mContext,newSourceUri,
                        sampleSize);*/
                // google will get Orientation from Exif,
                // it will be set orientaion as 0 if without Exif
    ...
    private final static int EDIT_PHOTO_SIZE_LIMIT = FeatureConfig.sIsLowRamDevice ? 5 * 1024 * 1024
            : 10 * 1024 * 1024;
    /// @}
}
