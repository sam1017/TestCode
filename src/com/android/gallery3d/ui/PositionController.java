...
/// @}
	...
	//Xiaoyh@Itel-Mobile modify from 4 to 0.375f for scale image at 2016/09/01
    private static final float SCALE_LIMIT =  4;
	//End
    public float MAX_SCALE_LIMIT = 2.0f;
    ...
    */
    /**
     * Add parameter MediaType[] for plug-in.
     */
    public void moveBox(int fromIndex[], boolean hasPrev, boolean hasNext,
            boolean constrained, Size[] sizes, MediaType[] types) {
    /// @}
        ...
        for (int i = -BOX_MAX; i <= BOX_MAX; i++) {
            ...
            GalleryPluginUtils.getImageOptionsPlugin().updateBoxMediaType(
                    PositionController.this, i, types[i + BOX_MAX]);
            /// @}
    ...
    private float getMinimalScale(Box b) {
        ...
        /// M: [FEATURE.ADD] plugin @{
        s = GalleryPluginUtils.getImageOptionsPlugin().getMinScaleLimit(b.mMediaType, s);
        /// @}
    ...
    /// M: [FEATURE.ADD] plugin @{
    /**
     * Set MediaType of box.
     */
    public void setMediaType(int index, MediaType mediaType) {
        Box b = mBoxes.get(index);
        if (mediaType == b.mMediaType) {
            return;
        }
        b.mMediaType = mediaType;
    }
    /// @}
    public float getImageMinScale() {
        ...
        if (b == null)
            return 1.0f;
        b.mScaleMax = b.mScaleMin * MAX_SCALE_LIMIT;
        return b.mScaleMin;
    }
}
