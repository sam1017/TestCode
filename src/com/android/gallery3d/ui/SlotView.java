...
/// @}
    ...
    //private static final boolean WIDE = mIsFancyLayoutSupported ? true : false;
    ...
    /// @}
        ...
        private void initLayoutParameters(
			...
			padding[0] = mSpec.horizontalPadding;
			/* modify by liangchangwei 2016-9-9--*/
            ...
            if (mSpec.slotWidth != -1) {
                ...
                */
                if (mIsFancyLayoutSupported) {
                    ...
                    int rows = (mWidth > mHeight) ? mSpec.rowsLand : mSpec.rowsPort;
                    mSlotGap = mSpec.slotGap;
                    mSlotWidth = Math.max(1, (mWidth - (rows - 1) * mSlotGap - 2 * mSpec.horizontalPadding) / rows);
                    mSlotHeight = mSlotWidth;
                    //mSlotHeight = Math.max(1, (mHeight - (rows - 1) * mSlotGap) / rows);
                    //mSlotWidth = mSlotHeight - mSpec.slotHeightAdditional;
					/*--modify by liangchangwei 2016-9-9--*/
    ...
    public void switchLayout(int whichLayout) {
        ...
        switch (whichLayout) {
            case FANCY_LAYOUT:
                if (mLayout != null && mLayout.isFancyLayout()) {
                    return;
                }
    ...
    private void addSlot(int index, MediaItem item, int size, boolean isCameraFolder,
        ...
        if (mediaType == MediaType.CONTAINER) {
            if (rotation == 90 || rotation == 270) {
                slotEntry = new SlotEntry(index, fancyOritation,
                        item.getHeight(), item.getWidth(),
                        item, rotation, slotWidth, slotGap,
                        isCameraFolder, albumName);
            } else {
                slotEntry = new SlotEntry(index, fancyOritation,
            ...
            slotEntry = new SlotEntry(index, fancyOritation, width, height, item, rotation,
                    slotWidth, slotGap, isCameraFolder, albumName);
        } else {
            ...
            if (rotation == 90 || rotation == 270) {
                if ((float) item.getWidth() / (float) item.getHeight()
                        > FancyHelper.FANCY_CROP_RATIO) {
                    constrainedWidth = Math.round((float) item.getHeight()
                            * FancyHelper.FANCY_CROP_RATIO);
                } /*
                   * else if ((float)item.getWidth() / (float)item.getHeight() <
                   * FancyHelper.FANCY_CROP_RATIO_LAND){ // picture will be
                   * recognized as panorama, so may not consider this situation
                   * constrainedHeight = Math.round((float)item.getWidth() /
                   * FancyHelper.FANCY_CROP_RATIO_LAND); }
                   */
                slotEntry = new SlotEntry(index, fancyOritation, constrainedHeight,
                        constrainedWidth, item, rotation, slotWidth, slotGap, isCameraFolder,
                        albumName);
            } else {
                if ((float) item.getHeight() / (float) item.getWidth()
                        > FancyHelper.FANCY_CROP_RATIO) {
                    constrainedHeight = Math.round((float) item.getWidth()
                            * FancyHelper.FANCY_CROP_RATIO);
                } /*
                   * else if ((float)item.getHeight() / (float)item.getWidth() <
                   * FancyHelper.FANCY_CROP_RATIO_LAND) { // picture will be
                   * recognized as panorama, so may not consider this situation
                   * constrainedWidth = Math.round((float)item.getHeight() /
                   * FancyHelper.FANCY_CROP_RATIO_LAND); }
                   */
                slotEntry = new SlotEntry(index, fancyOritation, constrainedWidth,
                        constrainedHeight, item, rotation, slotWidth, slotGap, isCameraFolder,
                        albumName);
            }
        }
