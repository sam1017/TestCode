...
public class GLRootView extends GLSurfaceView
    ...
    public GLRootView(Context context) {
        this(context, null);
        /// M: [DEBUG.ADD] @{
        TraceHelper.traceBegin(">>>>GLRootView-new1-done");
    ...
    public GLRootView(Context context, AttributeSet attrs) {
        super(context, attrs);
        /// M: [DEBUG.ADD] @{
        TraceHelper.traceBegin(">>>>GLRootView-new2-super-done");
        TraceHelper.traceEnd();
        TraceHelper.traceBegin(">>>>GLRootView-new2-other");
        /// @}
        ...
        */
        ...
        if (FeatureConfig.SUPPORT_EMULATOR) {
            ...
            /// M: [DEBUG.ADD] @{
            TraceHelper.traceBegin(">>>>GLRootView-new2-eglGetDisplay");
            /// @}
            EGLDisplay eglDisplay = egl.eglGetDisplay(EGL10.EGL_DEFAULT_DISPLAY);
            /// M: [DEBUG.ADD] @{
            TraceHelper.traceEnd();
            TraceHelper.traceBegin(">>>>GLRootView-new2-eglInitialize");
            /// @}
            ...
            Log.d(TAG, "ro.kernel.qemu.gles = " + "1"
                    .equals(SystemProperties.get("ro.kernel.qemu.gles"))
                        + ", supportSW3D = " + mSupportSW3D);
            /// M: [DEBUG.ADD] @{
            TraceHelper.traceBegin(">>>>GLRootView-new2-setEGLContextClientVersion");
            /// @}
            setEGLContextClientVersion((ApiHelper.HAS_GLES20_REQUIRED && (mSupportSW3D || "1"
                    .equals(SystemProperties.get("ro.kernel.qemu.gles")))) ? 2 : 1);
            /// M: [DEBUG.ADD] @{
            TraceHelper.traceEnd();
            /// @}
        } else {
        ...
        if (ApiHelper.USE_888_PIXEL_FORMAT) {
            /// M: [DEBUG.ADD] @{
            TraceHelper.traceBegin(">>>>GLRootView-new2-setEGLConfigChooser");
            /// @}
        ...
        /// M: [DEBUG.ADD] @{
        TraceHelper.traceBegin(">>>>GLRootView-new2-setRenderer");
        /// @}
        setRenderer(this);
        /// M: [DEBUG.ADD] @{
        TraceHelper.traceEnd();
        ...
        if (ApiHelper.USE_888_PIXEL_FORMAT) {
            /// M: [DEBUG.ADD] @{
            TraceHelper.traceBegin(">>>>GLRootView-new2-setFormat");
            /// @}
    ...
    protected void onLayout(
        ...
        /// M: [DEBUG.ADD] @{
        TraceHelper.traceBegin(">>>>GLRootView-onLayout");
        /// @}
    ...
    public void onSurfaceCreated(GL10 gl1, EGLConfig config) {
        /// M: [DEBUG.ADD] @{
        TraceHelper.traceBegin(">>>>GLRootView-onSurfaceCreated");
        Log.i(TAG, "<onSurfaceCreated> gl1 = " + gl1);
        /// @}
        GL11 gl = (GL11) gl1;
        if (mGL != null) {
        ...
        try {
            ...
            if (FeatureConfig.SUPPORT_EMULATOR) {
                mCanvas = (ApiHelper.HAS_GLES20_REQUIRED && (mSupportSW3D || "1"
                        .equals(SystemProperties.get("ro.kernel.qemu.gles")))) ? new GLES20Canvas()
                        : new GLES11Canvas(gl);
            } else {
    ...
    public void onSurfaceChanged(GL10 gl1, int width, int height) {
        Log.i(TAG, "onSurfaceChanged: " + width + "x" + height
                + ", gl10: " + gl1.toString());
        /// M: [DEBUG.ADD] @{
        TraceHelper.traceBegin(">>>>GLRootView-onSurfaceChanged");
        /// @}
    ...
    private void outputFps() {
        ...
        /// M: [TESTCASE.ADD] @{
            ...
            Log.i("Gallery2PerformanceTestCase2", "[Performance Auto Test] Gallery render fps = "
    ...
    public void onDrawFrame(GL10 gl) {
        ...
        if (!sVideoThumbnailPlaying) {
            TraceHelper.traceCounterForLaunchPerf(1);
            TraceHelper.traceCounterForLaunchPerf(0);
        }
        /// @}
    }
...
//********************************************************************
    ...
    public static boolean sVideoThumbnailPlaying = false;
    ...
    public void setFpsPerformance(boolean open) {
        Log.i(TAG, "[Performance Auto Test] setFpsPerformance = " + open);
    ...
    public class DebugThread extends Thread {
        ...
        @Override
        public void run() {
            Log.i(TAG, "<DebugThread> start");
            try {
                while (true) {
                    mTime++;
                    Log.i(TAG, "<DebugThread> time = " + mTime + ", begin #######################");
                    Log.i(TAG, "<DebugThread> mRenderLock = " + mRenderLock);
                    Log.i(TAG, "<DebugThread> mRenderLock.getHoldCount() = " +
                    mRenderLock.getHoldCount());
                    Log.i(TAG, "<DebugThread> mRenderLock.getQueueLength() = " +
                    mRenderLock.getQueueLength());
                    Log.i(TAG, "<DebugThread> mRenderLock.hasQueuedThreads() = " +
                    mRenderLock.hasQueuedThreads());
                    Log.i(TAG, "<DebugThread> mRenderLock.isFair() = " + mRenderLock.isFair());
                    Log.i(TAG, "<DebugThread> mRenderLock.isHeldByCurrentThread() = " +
                    mRenderLock.isHeldByCurrentThread());
                    Log.i(TAG, "<DebugThread> mRenderLock.isLocked() = " + mRenderLock.isLocked());
                    Log.i(TAG, "<DebugThread> time = " + mTime + ", end #########################");
                    Thread.sleep(1000);
                }
            } catch (InterruptedException e) {
                Log.i(TAG, "<DebugThread> Interrupted, " + e.getMessage());
            }
            Log.i(TAG, "<DebugThread> stop");
        }
    }
    ...
    public void startDebug() {
        ...
        mDebugThread.start();
        Log.i(TAG, "<startDebug> new DebugThread");
    }
    ...
    public void stopDebug() {
        ...
        Log.i(TAG, "<stopDebug> interrupt DebugThread");
    }
    /// @}
}
