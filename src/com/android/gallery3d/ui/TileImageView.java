...
public class TileImageView extends GLView {
    ...
    private class TileUploader implements GLRoot.OnGLIdleListener {
        ...
        /// M: [DEBUG.ADD] @{
        boolean mDebugTileUploader;
        public TileUploader() {
            mDebugTileUploader = (SystemProperties.getInt("gallery.debug.renderlock", 0) == 1);
        }
        /// @}
        @Override
        public boolean onGLIdle(GLCanvas canvas, boolean renderRequested) {
            if (mDebugTileUploader) {
                Log.i(TAG, "<TileUploader.onGLIdle> begin");
            }
            ...
            if (renderRequested) {
                if (mDebugTileUploader) {
                    Log.i(TAG, "<TileUploader.onGLIdle> renderRequested, return");
                }
                return true;
            }
            /// @}
            ...
            while (quota > 0) {
                /// M: [DEBUG.ADD] @{
                if (mDebugTileUploader) {
                    Log.i(TAG, "<TileUploader.onGLIdle> [while] quota = " + quota);
                }
                /// @}
                ...
                if (tile == null) {
                    if (mDebugTileUploader) {
                        Log.i(TAG, "<TileUploader.onGLIdle> [while] tile is null, break");
                    }
                    break;
                }
                /// @}
                if (!tile.isContentValid()) {
                    ...
                    if (tile.mTileState != STATE_DECODED) {
                        if (mDebugTileUploader) {
                            Log.i(TAG, "<TileUploader.onGLIdle> [while] tile not DECODED, break");
                        }
                        break;
                    }
                    ///}@
                ...
                /// M: [DEBUG.ADD] @{
                else if (mDebugTileUploader) {
                    Log.i(TAG, "<TileUploader.onGLIdle> [while] tile is valid, do nothing");
                }
                /// @}
            }
            ...
            /// M: [DEBUG.ADD] @{
            if (mDebugTileUploader) {
                Log.i(TAG, "<TileUploader.onGLIdle> return " + (tile != null));
            }
            /// @}
            return tile != null;
        }
            ...
            while (!isInterrupted()) {
                ...
                synchronized (TileImageView.this) {
                    ...
                    if (tile == null && !isInterrupted()) {
                        Log.i(TAG, "<TileDecoder.run> wait, this = " + TileDecoder.this);
                        try {
                ...
                Log.i(TAG, "<TileDecoder.run> decodeTile, this = " + TileDecoder.this);
                if (decodeTile(tile)) {
                    queueForUpload(tile);
                }
            }
            Log.i(TAG, "<TileDecoder.run> exit, this = " + TileDecoder.this);
        }
    }
    /// @}
