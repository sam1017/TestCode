...
/**
 * implements PlayEngine.OnFrameAvailableListener for auto play
 */
...
/// @}
    ...
    public interface Listener {
        ...
        /// M: [FEATURE.ADD] @{
        /**
         * Call back when single tap is confirmed.
         * @param x
         * @param y
         */
    ...
    //add by liangchangwei 2016-9-20- for scale limit
    private static final float MAX_SCALE_LIMIT = 2.0f; // scale from initscale
    ...
    public PhotoView(AbstractGalleryActivity activity) {
        ...
        if (FeatureConfig.SUPPORT_SLOW_MOTION
    ...
    /// @}
        ...
        /// M: [FEATURE.ADD] plugin @{
        /**
         * Update the MediaType of picture.
         * @param mediaType
         */
        public void updateMediaType(MediaType mediaType);
        /**
         * Get MediaType of picture.
         * @return the MediaType of picture
         */
    ...
    class FullPicture implements Picture {
        ...
        private void drawTileView(GLCanvas canvas, Rect r) {
            ...
            MediaData.MediaType type = (data != null) ? data.mediaType : MediaType.INVALID;
            if (type == MediaType.INVALID) {
                Log.i(TAG, "<FullPicture.drawTileView> item " + item + ", data " + data + ", type "
                        + type);
            }
            if ((mIsVideo && !FeatureConfig.SUPPORT_SLIDE_VIDEO_PLAY)
    ...
    private class MyGestureListener implements GestureRecognizer.Listener {
        ...
        public boolean onDoubleTap(float x, float y) {
            ...
            mIgnoreUpEvent = true;
            if (scale <= .45f || controller.isAtMinimalScale()) {
                /// M: [BUG.MODIFY] A black area show at the right side of picture.@{
                /* controller.zoomIn(x, y, Math.max(1.0f, scale * 1.5f)); */
                //modify by liangchangwei 2016-9-20- for scale limit
                //controller.zoomIn(x, y, Math.max(1.5f, Math.min(scale * 1.5f, MAX_SCALE_LIMIT)));
                controller.zoomIn(x, y, Math.min(1.5f, scale * MAX_SCALE_LIMIT));
                /// @}
            } else {
        ...
        public boolean onScale(float focusX, float focusY, float scale) {
            ...
            boolean largeEnough = (mAccScale < 0.5f || mAccScale > 1.5f);

            //add by liangchangwei 2016-9-20- for scale limit
            float currScale = mPositionController.getImageScale();
            if(currScale > (mPositionController.getImageMinScale()) * MAX_SCALE_LIMIT){
                largeEnough  = true;
            }
        ...
        /// M: [BEHAVIOR.ADD] @{
        /**
         * set mIgnoreScalingGesture flag as !enabled.
         * @param enabled
         */
    ...
    /// M: [BEHAVIOR.ADD] @{
    /**
     * tell GestureListener if scale is end.
     * @param enabled
     */
    public void setScalingEnabled(boolean enabled) {
    ...
    private void switchToHitPicture(int x, int y) {
        ...
        Log.d(TAG, "<switchToHitPicture> x=" + x + ", y=" + y + ", hit test result index="
                + hitIndex);
            ...
            if (curIndex + hitIndex < 0) {
                ...
                if (mModel instanceof PhotoDataAdapter) {
                    ...
                    if (totalCount > 0 && (curIndex + hitIndex) >= totalCount) {
                        Log.i(TAG, "<switchToHitPicture> adjust targetIndex from " + targetIndex
                                + " to " + (totalCount - 1));
...
//        }
    ...
    /**
     * Call back when create option menu
     * @param menu
     * @return
     */
    ...
    /**
     * Call back when prepare options menu
     * @param menu
     * @return
     */
    ...
    /**
     * Call back when option item is selected
     * @param item
     * @return
     */
    ...
    /**
     * Call back when visibility of action bar changed
     * @param newVisibility
     * @return
     */
    ...
    /**
     * Set IBackwardContoller to LayerManager.
     * @param backwardControllerForLayer
     */
