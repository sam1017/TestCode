...
import com.mediatek.gallery3d.adapter.ContainerSource;
...
public class DataManager implements StitchingChangeListener {
    ...
    public synchronized void initializeSourceMap() {
        ...
        /// M: [FEATURE.ADD] @{
        addSource(new ContainerSource(mApplication));
        /// @}
    ...
    public MediaObject getMediaObject(Path path) {
        synchronized (LOCK) {
            ...
            Log.w(TAG,"getMediaObject path = " + path + " obj = " + obj);
            if (obj != null) return obj;

            MediaSource source = mSourceMap.get(path.getPrefix());
            Log.w(TAG,"getMediaObject source = " + source + " path.getPrefix() = " + path.getPrefix());
            if (source == null) {
