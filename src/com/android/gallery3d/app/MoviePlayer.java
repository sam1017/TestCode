...
public class MoviePlayer implements
    ...
    ///M: for audofocus change, keep current state when loss audio focus
    private TState mKeepState = TState.PLAYING;
    ...
    public MoviePlayer(View rootView, final MovieActivity movieActivity, IMovieItem info,
        ...
        mHideSystemUIRunnable = new Runnable() {
            ...
            public void run() {
                ...
                if (mWfdPowerSaving != null && !mHasPaused
...
//        });
        ...
        if (MovieUtils.isWfdEnabled(mContext)) {
            if (mWfdPowerSaving != null) {
                if (mActivityContext.equals(mWfdPowerSaving.getCurrentActivity())) {
                    ...
                    mWfdPowerSaving =
                            new WfdPowerSaving(mRootView, mActivityContext, mVideoView, mHandler) {
                        @Override
  ...
  /// @}
    ...
    private void doStartVideo(
            final boolean enableFasten, final int position, final int duration, boolean start) {
        if (LOG) {
            Log.v(TAG, "doStartVideo("
                + enableFasten + ", " + position + ", " + duration + ", " + start + ")");
        }
    ...
    public void onAudioFocusChange(int focusChange){
        if (LOG) {
            Log.v(TAG, "AudioFocusChange state is " + focusChange + " mKeepState = "
                + mKeepState + " mTState = " + mTState + " mIsBuffering = " + mIsBuffering);
        }
        if (focusChange == AudioManager.AUDIOFOCUS_GAIN) {
            if (mKeepState == TState.PLAYING && !mIsBuffering) {
                playVideo();
            }
    ...
    public boolean onError(MediaPlayer player, int arg1, int arg2) {
        ...
        if (mController == null && FeatureConfig.SUPPORT_SLIDE_VIDEO_PLAY) {
            mActivityContext.runOnUiThread(new Runnable() {
                @Override
                public void run() {
    ...
    public void onSeekMove(int time) {
        ...
        // /M: If support preroll, keep google seek behavior
    ...
    private class WfdConnectReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            if (intent != null &&
                    intent.getAction().equals(DisplayManager.ACTION_WIFI_DISPLAY_STATUS_CHANGED)) {
                WifiDisplayStatus status = (WifiDisplayStatus) intent.getParcelableExtra(
                        DisplayManager.EXTRA_WIFI_DISPLAY_STATUS);
                Log.v(TAG, "mWfdReceiver onReceive wfd status: " + status);
                if (status != null &&
                    status.getActiveDisplayState() == WifiDisplayStatus.DISPLAY_STATE_CONNECTED) {
                    // WFD connected
                    Toast.makeText(mContext.getApplicationContext(),
                        mContext.getString(R.string.wfd_connected), Toast.LENGTH_LONG).show();
                    mActivityContext.finish();
                } else {
                   // WFD disconnected of WFD is connecting
                }
            }
        }
...
//        });
    ...
    public void onPrepared(MediaPlayer mp) {
        ...
        if (MtkVideoFeature.isSubTitleSupport()) {
            addExternalSubtitle();
            if (getTrackNumFromVideoView(MediaPlayer.TrackInfo.MEDIA_TRACK_TYPE_TIMEDTEXT)
                    < minTrackNumber) {
        ...
        if (MtkVideoFeature.isAudioChangeSupport() && mVideoType == MovieUtils.VIDEO_TYPE_LOCAL) {
            if (getTrackNumFromVideoView(MediaPlayer.TrackInfo.MEDIA_TRACK_TYPE_AUDIO)
                    < minTrackNumber) {
    ...
    public void onBufferingUpdate(MediaPlayer mp, int percent) {
        ...
        if (LOG) {
            Log.v(TAG, "onBufferingUpdate(" + percent + ") pauseBuffering="
    ...
    private void doStartVideoCareDrm(
            final boolean enableFasten, final int position, final int duration) {
        if (LOG) {
            Log.v(TAG, "doStartVideoCareDrm("
                + enableFasten + ", " + position + ", " + duration + ")");
        }
        mTState = TState.PLAYING;
    ...
    public void onVideoSizeChanged(MediaPlayer mp, int width, int height) {
        ...
        if (LOG) {
            Log.v(TAG, "onVideoSizeChanged("
                + width + ", " + height + ") mIsOnlyAudio=" + mIsOnlyAudio);
        }
    }
    /// @}
    ...
    private class RetryExtension implements
            Restorable, MediaPlayer.OnErrorListener, MediaPlayer.OnInfoListener {
    ...
    */
    ...
    public int getTrackNumFromVideoView(int type) {
        ...
        for (int i = 0; i < trackLength; i++) {
            if (trackInfo[i].getTrackType() == MediaPlayer.TrackInfo.MEDIA_TRACK_TYPE_AUDIO) {
                AudioNum++;
                continue;
            } else if (trackInfo[i].getTrackType()
                    == MediaPlayer.TrackInfo.MEDIA_TRACK_TYPE_TIMEDTEXT) {
                SubtilteNum++;
                continue;
            }
    ...
    private void addExternalSubtitle() {
        File[] externalSubTitlefiles
            = ((MovieActivity) mActivityContext).listExtSubTitleFileNameWithPath();
        if (null != externalSubTitlefiles) {
            ...
            for (File file : externalSubTitlefiles) {
                ...
                if (filePath.endsWith(MovieUtils.SUBTITLE_SUPPORT_WITH_SUFFIX_SRT.
                           toLowerCase())) {
                    ((MTKVideoView) mVideoView).addExtTimedTextSource(
                        file.getPath(), MediaPlayer.MEDIA_MIMETYPE_TEXT_SUBRIP);
                    continue;
                } else if (filePath.endsWith(MovieUtils.SUBTITLE_SUPPORT_WITH_SUFFIX_SMI.
                           toLowerCase())) {
                    ((MTKVideoView) mVideoView).addExtTimedTextSource(
                        file.getPath(), MovieUtils.MEDIA_MIMETYPE_TEXT_SUBSMI);
                    continue;
                } else if (filePath.endsWith(MovieUtils.SUBTITLE_SUPPORT_WITH_SUFFIX_SUB.
                           toLowerCase())) {
                    ((MTKVideoView) mVideoView).addExtTimedTextSource(
                        file.getPath(), MovieUtils.MEDIA_MIMETYPE_TEXT_SUB);
                    continue;
                } else if (filePath.endsWith(MovieUtils.SUBTITLE_SUPPORT_WITH_SUFFIX_IDX.
                           toLowerCase())) {
                    ((MTKVideoView) mVideoView).addExtTimedTextSource(
                        file.getPath(), MovieUtils.MEDIA_MIMETYPE_TEXT_IDX);
                    continue;
                } else if (filePath.endsWith(MovieUtils.SUBTITLE_SUPPORT_WITH_SUFFIX_TXT.
                           toLowerCase())) {
                    ((MTKVideoView) mVideoView).addExtTimedTextSource(
                        file.getPath(), MovieUtils.MEDIA_MIMETYPE_TEXT_SUBTXT);
                    continue;
                } else if (filePath.endsWith(MovieUtils.SUBTITLE_SUPPORT_WITH_SUFFIX_ASS.
                           toLowerCase())) {
                    ((MTKVideoView) mVideoView).addExtTimedTextSource(
                        file.getPath(), MovieUtils.MEDIA_MIMETYPE_TEXT_SUBASS);
                    continue;
                } else if (filePath.endsWith(MovieUtils.SUBTITLE_SUPPORT_WITH_SUFFIX_SSA.
                           toLowerCase())) {
                    ((MTKVideoView) mVideoView).addExtTimedTextSource(
                        file.getPath(), MovieUtils.MEDIA_MIMETYPE_TEXT_SUBSSA);
                    continue;
                } else if (filePath.endsWith(MovieUtils.SUBTITLE_SUPPORT_WITH_SUFFIX_MPL.
                          toLowerCase())) {
                    ((MTKVideoView) mVideoView).addExtTimedTextSource(
                        file.getPath(), MovieUtils.MEDIA_MIMETYPE_TEXT_SUBMPL);
                    continue;
                }
            }
        }
        //selsect the bookMark saved track.
        //if mSelectSubTitleTrackIdx > hasTrackNum do not call this
                    ...
                    new DialogInterface.OnClickListener() {
                        ...
                        Log.v(TAG, "AudioAndSubTitleChange  onClick whichButton = " + whichButton);
                            ...
                            if (RETURN_ERROR ==
                                ...
                                if (mTState == TState.PAUSED) {
                                    Log.v(TAG, "AudioAndSubTitleChange"
                                        + " --- onClick if has error after selectTrack");
                                    playVideo();
                                    mVideoView.selectTrack(TrackIdx[whichButton].intValue());
                                    pauseVideo();
                                }
                    ...
                    new DialogInterface.OnClickListener() {
                        public void onClick(DialogInterface dialog,
                            ...
                            if (mSelectSubTitleIndex == whichButton) {
                                Log.v(TAG, "AudioAndSubTitleChange "
                                    + "--- onClick whichButton SameChoice");
                                dialog.dismiss();
                            } else {
                                if (whichButton == 0) {
                                    mVideoView.deselectTrack(
                                        TrackIdx[mSelectSubTitleIndex].intValue());
                                    updateSubtitleView(null);
   ...
   ///@}
    ...
    public class MoviePlayerExtension implements IMoviePlayer, Restorable {
        ...
        public void addBookmark() {
            ...
            if (mBookmark.exists(uri)) {
                Toast.makeText(
                    mActivityContext, R.string.bookmark_exist, Toast.LENGTH_SHORT).show();
            } else {
                mBookmark.insert(mTitle, uri, mMovieItem.getMimeType(), 0);
                Toast.makeText(
                    mActivityContext, R.string.bookmark_add_success, Toast.LENGTH_SHORT).show();
            }
            if (LOG) {
            ...
            if (next != null && next != mMovieItem) {
                ...
                if (MtkVideoFeature.isAudioChangeSupport() || MtkVideoFeature.isSubTitleSupport()) {
                    mBookmarker.setBookmark(mMovieItem.getUri(), position, duration,
                        mSelcetAudioTrackIdx, mSelectSubTitleTrackIdx,
                        mSelectAudioIndex, mSelectSubTitleIndex);
                } else {
                    mBookmarker.setBookmark(mMovieItem.getUri(), position, duration);
                }
                //mBookmarker.setBookmark(mMovieItem.getUri(), position, duration);
    ...
    public class SlideVideoExt implements IVideoPlayer {
        ...
        public void setMovieControllerListener() {
            ...
            mAudioBecomingNoisyReceiver.register();
            /// M: [FEATURE.ADD] add for slow motion,
            /// do not movie these codes to runOnUiThread().@{
            if (mController == null) {
                mController = MovieControllerOverlay.getMovieController(mContext);
            }
        ...
        public void run() {
            ...
            if (mWfdPowerSaving != null) {
                ...
                if (!mWfdPowerSaving.needShowController()
                        && !MtkVideoFeature.isEnableMultiWindow()) {
                    mRootView.setBackground(null);
                }
...
class Bookmarker {
    ...
    ///@{
    public void setBookmark(Uri uri, int bookmark, int duration,
                            int audioIdx, int subtitleIdx,
                            int audioDlgListIdx, int  subtitleDlgListIdx) {
        if (LOG) {
            Log.v(TAG, "setBookmark(" + uri + ", " + bookmark + ", " + duration + ", "
                    + audioIdx + ", " + subtitleIdx + ", " + audioDlgListIdx
                    + ", " + subtitleDlgListIdx + ")");
        }
        try {
            //do not record or override bookmark if duration is not valid.
    ...
    public BookmarkerInfo getBookmark(Uri uri) {
        try {
            ...
            if (data == null) {
                if (LOG) {
                    Log.v(TAG, "getBookmark("
                        + uri + ") data=null. uri.hashCode()=" + uri.hashCode());
                }
                return null;
            }
            ...
            if (LOG) {
                Log.v(TAG, "getBookmark(" + uri + ") uriString="
                        + uriString + ", bookmark=" + bookmark
                        + ", duration=" + duration);
            }
            ...
            if (MtkVideoFeature.isAudioChangeSupport() || MtkVideoFeature.isSubTitleSupport()) {
                ...
                return new BookmarkerInfo(
                    bookmark, duration, audioIdx, subtitleIdx, audioDlgListIdx, subtitleDlgListIdx);
            }
            else {
