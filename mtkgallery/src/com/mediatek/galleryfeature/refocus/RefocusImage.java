...
public class RefocusImage {
    ...
    public Uri saveRefocusImage(Uri sourceUri) {
        ...
        String filename = new SimpleDateFormat(TIME_STAMP_NAME).format(new Date(System
                .currentTimeMillis()));
        File file = RefocusHelper
                .getNewFile(mActivity.getApplicationContext(), sourceUri, filename);
        long begin = System.currentTimeMillis();
        RefocusImageJNI.saveRefocusImage(file.getAbsolutePath(), INSAMPLESIZE);
        Log.d(TAG, "performance saveRefocusImage costs " + (System.currentTimeMillis() - begin));
        return RefocusHelper.insertContent(mActivity.getApplicationContext(), sourceUri, file,
                filename);
    }
