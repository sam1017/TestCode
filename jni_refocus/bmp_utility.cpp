...
int kernel[25] = { 1, 5, 8, 5, 1, 5, 20, 33, 20, 5, 8, 33, 54, 33, 8, 5, 20, 33, 20, 5, 1, 5, 8, 5, 1 };
...
void bmp_rgb2gray(BITMAP *image, BITMAP *gray_image) {
    ...
    for (j = 0; j < image->height; j++) {
        for (i = 0; i < image->width; i++) {
            index = image->width * j + i;
            float temp = 0.3f * (float) image->r[index] + 0.59f * (float) image->g[index]
                    + 0.11f * (float) image->b[index];
    ...
    for (y = 0; y <= (src->height - 1); y++) {
        for (x = 0; x <= (src->width - 1); x++) {
            ...
            /* Convolution starts here */
                ...
                for (I = -1; I <= 1; I++) {
                    for (J = -1; J <= 1; J++) {
                        sumX = sumX + (int) ((*(src->y + x + I + (y + J) * src->width)) * GX[I + 1][J + 1]);
                    }
                }
                ...
                for (I = -1; I <= 1; I++) {
                    for (J = -1; J <= 1; J++) {
                        sumY = sumY + (int) ((*(src->y + x + I + (y + J) * src->width)) * GY[I + 1][J + 1]);
                    }
                }
    ...
    for (y = 0; y < HEIGHT; y++) {
        for (x = 0; x < WIDTH; x++) {
            ...
            if (x % 2 == 1 && y % 2 == 1) {
                j = (WIDTH >> 1) * (y >> 1) + (x >> 1);
                U[j] =
                        (unsigned char) ((DU(*R, *G, *B) + DU(*(R-1), *(G-1), *(B-1))
                                + DU(*(R-WIDTH), *(G-WIDTH), *(B-WIDTH))
                                + DU(*(R-WIDTH-1), *(G-WIDTH-1), *(B-WIDTH-1))) / 4);

                V[j] =
                        (unsigned char) ((DV(*R, *G, *B) + DV(*(R-1), *(G-1), *(B-1))
...
double YuvToRgb[3][3] = { { 1, 0.0000, 1.4022 }, { 1, -0.3456, -0.7145 }, { 1, 1.7710, 0.0000 } };
...
void YUV420_toBMP(kal_uint8 *src, BITMAP *dest) {
    ...
    for (y = 0; y < height; y++) {
        for (x = 0; x < width; x++) {
            // r component
            temp = (kal_int32)(
                    cTemp[0][y * width + x] + (cTemp[2][(y / 2) * (width / 2) + x / 2] - 128) * YuvToRgb[0][2]);
            cTemp[3][y * width + x] = temp < 0 ? 0 : (temp > 255 ? 255 : temp);
            // g component
            temp = (kal_int32)(
                    cTemp[0][y * width + x] + (cTemp[1][(y / 2) * (width / 2) + x / 2] - 128) * YuvToRgb[1][1]
                            + (cTemp[2][(y / 2) * (width / 2) + x / 2] - 128) * YuvToRgb[1][2]);
            cTemp[4][y * width + x] = temp < 0 ? 0 : (temp > 255 ? 255 : temp);
            // b component
            temp = (kal_int32)(
                    cTemp[0][y * width + x] + (cTemp[1][(y / 2) * (width / 2) + x / 2] - 128) * YuvToRgb[2][1]);
